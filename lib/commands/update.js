...
const { writeScaffoldMeta } = require('./org-profile');

function updateOne(infraDir, targetDir, registryPath, tuning) {
  ...
  copy.deploySettings(targetDir, tuning, entry.model_profile || entry.model);
  ...
  registerDeploy(
    ...
    targetDir,
    {
      ...
      modelProfile: entry.model_profile || entry.model,
      baseProfile: entry.base_profile,
    },
    registryPath
  );
...
function updateAll(infraDir, registryPath, tuning) {
  const registry = loadRegistry(registryPath);
  ...
  for (const entry of registry.deployed) {
    ...
    if (repoSha && currentSha && repoSha === currentSha) {
      skipped++;
      continue;
    }

    if (!fs.existsSync(entry.path)) {
      process.stderr.write(`[update] Skipping ${entry.path} — directory not found\n`);
      skipped++;
      continue;
    }

    updateOne(infraDir, entry.path, registryPath, tuning);
    updated++;
  }
