...
function getOrgProfiles(infraDir) {
  const { listOrgProfiles } = require('../commands/org-profile');
  try {
    return listOrgProfiles(infraDir);
  } catch {
    return [];
  }
}

function runCliCommand(infraDir, args) {
  const result = spawnSync(
    process.execPath,
    [path.join(infraDir, 'bin', 'cli.js'), ...args],
    { encoding: 'utf8', timeout: 30000 }
  );
  const out = (result.stdout || '').trim();
  const err = (result.stderr || '').trim();
  return { out, err, ok: result.status === 0 && !result.error };
}
