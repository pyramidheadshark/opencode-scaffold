...
class AgentPool extends EventEmitter {
  ...
  spawn(targetDir, task) {
    ...
    const FRAME_RESET_RE = /\x1b\[(?:2J|\?1049h)/;

    ptyProcess.onData(data => {
      agent.output += data;

      if (FRAME_RESET_RE.test(data)) {
        const idx = data.search(FRAME_RESET_RE);
        agent.displayBuffer = data.slice(idx);
      } else {
        agent.displayBuffer += data;
      }
