...
const { MODEL_IDS } = require('../models');
...
function buildHooksDefinition(targetDir) {
  ...
  return {
    ...
    PostToolUse:      [{ matcher: 'Bash|Edit|Write', hooks: [{ type: 'command', command: `node "${h}/post-tool-use-tracker.js"` }] }, { matcher: '.*', hooks: [{ type: 'command', command: `node "${h}/session-checkpoint.js"` }] }],
...
function effortDefaultForProfile(modelProfile) {
  if (modelProfile === 'opus') return 'medium';
  return 'max';
}

function applyTuningDefaults(existing, tuning, modelProfile, provider) {
  const t = { ...DEFAULT_TUNING, ...(tuning || {}) };
  existing.env = existing.env || {};

  const modelEffort = effortDefaultForProfile(modelProfile);
  const effortToUse = (t.effort !== undefined && t.effort !== 'max') ? t.effort : modelEffort;

  if (existing.env.CLAUDE_CODE_DISABLE_1M_CONTEXT === undefined) {
    existing.env.CLAUDE_CODE_DISABLE_1M_CONTEXT = '1';
  }
  if (effortToUse !== 'off' && existing.env.CLAUDE_CODE_EFFORT_LEVEL === undefined) {
    existing.env.CLAUDE_CODE_EFFORT_LEVEL = effortToUse;
  }
  ...
  // CCR / OpenRouter provider env vars
  if (provider === 'openrouter') {
    if (existing.env.CLAUDE_CODE_DISABLE_EXPERIMENTAL_BETAS === undefined) {
      existing.env.CLAUDE_CODE_DISABLE_EXPERIMENTAL_BETAS = '1';
    }
    if (existing.env.ANTHROPIC_BASE_URL === undefined) {
      existing.env.ANTHROPIC_BASE_URL = 'http://127.0.0.1:3456';
    }
    if (existing.env.ANTHROPIC_AUTH_TOKEN === undefined) {
      existing.env.ANTHROPIC_AUTH_TOKEN = '$OPENROUTER_API_KEY';
    }
    // ANTHROPIC_API_KEY MUST be empty string — not null/undefined
    // If null/undefined, Claude Code ignores ANTHROPIC_AUTH_TOKEN and tries OAuth
    if (existing.env.ANTHROPIC_API_KEY === undefined) {
...
function applyModelProfile(existing, modelProfile) {
  if (!modelProfile) return;
  const modelId = MODEL_IDS[modelProfile];
  if (!modelId) return;
  existing.model = modelId;
  if (modelProfile === 'haiku' && existing.env && existing.env.CLAUDE_CODE_EFFORT_LEVEL) {
    delete existing.env.CLAUDE_CODE_EFFORT_LEVEL;
  }
}

function deploySettings(targetDir, tuning, modelProfile, provider) {
  ...
  if (existing.statusLine === undefined) {
    const h = path.join(targetDir, '.claude', 'hooks').replace(/\\/g, '/');
    existing.statusLine = { type: 'command', command: `node "${h}/session-status-monitor.js"` };
  }

  applyTuningDefaults(existing, tuning, modelProfile, provider);
  applyModelProfile(existing, modelProfile);

  // Add scaffold metadata for CCR/OpenRouter
  if (provider === 'openrouter') {
    existing.scaffold = existing.scaffold || {};
    if (!existing.scaffold.provider) existing.scaffold.provider = 'openrouter';
    if (!existing.scaffold.ccrEnabled) existing.scaffold.ccrEnabled = true;
  }
