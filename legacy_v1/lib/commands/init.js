...
function deployCore(infraDir, targetDir, options) {
  ...
  if (options.dryRun) {
    ...
    process.stdout.write(`  Create .claude/settings.json (provider: ${provider})\n`);
    ...
    if (provider === 'openrouter') {
      process.stdout.write(`  Setup CCR: config.json + custom-router.js + preset\n`);
    }
  ...
  copy.createAgentExtensionsDir(targetDir);
  copy.mergeAgentExtensions(targetDir);
  copy.copyPitfalls(infraDir, targetDir);
  ...
  copy.deploySettings(targetDir, options.tuning, null, provider);
  copy.createDevStatus(infraDir, targetDir);
  copy.ensureGitignore(targetDir);

  // CCR / OpenRouter setup
  if (provider === 'openrouter') {
    const profile = options.profile || 'ai-developer';
    const ccrOptions = { force: options.force };

    // Check and install CCR
    if (!ccr.checkCCRInstalled()) {
      console.log('CCR not found. Installing...');
      ccr.installCCR();
    }

    // Deploy CCR config, custom router, and preset
    ccr.deployCCRConfig(profile, ccrOptions);
    ccr.deployCustomRouter(ccrOptions);
    ccr.deployCCRPreset(profile, ccrOptions);

    // Save scaffold mode
    ccr.writeScaffoldMode('default');

    console.log('✅ CCR/OpenRouter configured. Run `ccr start` before using Claude Code.');
  }
